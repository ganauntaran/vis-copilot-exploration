...
const Avatar: React.FC<AvatarProps> = ({ src, alt, className }) => {
  return (
    <img
      ...
      src={src}
      alt={alt}
      className={`shrink-0 w-10 aspect-square ${className}`}
    />
  );
};
...
const Logo: React.FC<LogoProps> = ({ src, alt, className }) => {
  return (
    <img
      ...
      src={src}
      alt={alt}
      className={`aspect-[1.85] w-[68px] ${className}`}
    />
  );
};
...
const NavItem: React.FC<NavItemProps> = ({ children, className }) => {
  return (
    <div
      className={`px-4 pt-2.5 pb-3 text-base font-medium text-center rounded-3xl text-white text-opacity-80 ${className}`}
    >
      {children}
    </div>
  );
        ...
        <header className="flex flex-col justify-center px-16 w-full bg-yellow-800 shadow-sm max-md:px-5 max-md:max-w-full">
          <div className="flex gap-5 justify-between px-8 py-5 mx-5 max-md:flex-wrap max-md:px-5 max-md:mr-2.5 max-md:max-w-full">
            <nav className="flex gap-5 justify-between">
              ...
              <NavItem className="bg-white bg-opacity-10 font-semibold">
                Pengaturan Akun
              </NavItem>
              <NavItem>Audit Log</NavItem>
            </nav>
            <div className="flex gap-5 justify-between rounded-3xl">
              <UserInfo name="Esther Howard" role="Admin Pusat" />
              <Avatar
